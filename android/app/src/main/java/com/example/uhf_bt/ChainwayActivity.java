...
public class ChainwayActivity extends BaseActivity implements View.OnClickListener {

    public boolean isScanning = false;
    public String remoteBTName = "";
    public String remoteBTAdd = "";
    private final static String TAG = "ChainwayActivity";
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (!isScanning) {
            if (item.getItemId() == R.id.UHF_Battery) {
                ...
                Utils.alert(ChainwayActivity.this, R.string.action_uhf_bat, ver, R.drawable.webtext);
            } else if (item.getItemId() == R.id.UHF_T) {
                String temp = getString(R.string.title_about_Temperature) + ":" + uhf.getTemperature() + "℃";
                Utils.alert(ChainwayActivity.this, R.string.title_about_Temperature, temp, R.drawable.webtext);
            } else if (item.getItemId() == R.id.UHF_ver) {
                String ver = uhf.getVersion();
                Utils.alert(ChainwayActivity.this, R.string.action_uhf_ver, ver, R.drawable.webtext);
                ...
                if (versionMap != null) {
                    ...
                    Utils.alert(ChainwayActivity.this, R.string.action_ble_ver, verMsg, R.drawable.webtext);
                }
            } else if (item.getItemId() == R.id.ble_disconnectTime) {
                View view = LayoutInflater.from(getApplicationContext()).inflate(R.layout.dialog_disconnect_time, null);
                ...
                //R2提取数据
                Intent intent = new Intent(getApplicationContext(),ExtractDataActivity.class);
        ...
        protected Boolean doInBackground(String... params) {
            ...
            try {
                ...
                for (int k = 0; !isSotp && k < size; k++) {
                    String epc=  tagList.get(k).get(ChainwayActivity.TAG_EPC);
                    String tid=  tagList.get(k).get(ChainwayActivity.TAG_TID);
                    String count=  tagList.get(k).get(ChainwayActivity.TAG_COUNT);
                    String rssi=  tagList.get(k).get(ChainwayActivity.TAG_RSSI);
